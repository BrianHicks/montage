    ...
    var action = new PlugIn.Action(async () => {
        try {
            ...
            req.bodyString = JSON.stringify({
                query: "mutation ExtendByMutation($duration: String!) { extendBy(duration: $duration) { projectedEndTime } }",
                variables: {
            ...
            let data = JSON.parse(resp.bodyString).data.extendBy;
            console.log(JSON.stringify(data));
            let endTime = new Date(data.projectedEndTime);
            new Alert("Extended session", `Extended sesion until ${endTime.getHours()}:${endTime.getMinutes()}`).show();
        }
        catch (err) {
            console.error(err);
        }
